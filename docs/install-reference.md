...
| `--cuda-torch` / `--cpu-torch` | `KENZY_CPU_TORCH` | auto | Which PyTorch build to install on a Linux/x86 host that needs one (the speaker service and the local Kokoro voice both use it). The default detects an NVIDIA card and installs the standard CUDA wheels only when one is present — otherwise the CPU-only build, which skips roughly **2.7 GB** of CUDA runtime the host cannot use. Force either way with these flags; a machine whose GPU isn't meant for Kenzy wants `--cpu-torch`. macOS and ARM hosts have no CUDA wheels and skip this entirely. If the CPU-only download fails the install continues with the standard build. **Existing installs keep the build they already have** — pip sees torch satisfied, so switching means removing the venv and reinstalling. |
