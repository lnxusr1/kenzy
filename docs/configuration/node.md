...
| `sound_thinking` | `"thinking.wav"` | Spoken acknowledgement ("On it.") when a reply is **taking a while** — played once if the model stage runs past a couple of seconds, so tool-heavy or slow-model requests aren't met with silence. Same server-side, live-apply semantics. Empty/`null` = never speak it. |
