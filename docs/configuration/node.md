...
| `sound_waiting` | `null` | WAV file played while waiting for the server response. **Loops** for the whole wait (bounded, so a dead server can't loop an empty room forever); the spoken processing cues below duck it underneath and it continues after them; it stops the instant the reply begins. `null` (or an empty string) disables it — pure silence while waiting. Provide a filename or path to enable it. |
...
| `sound_thinking` | `"thinking.wav"` | Spoken status note ("Working on it.") when a reply is **taking a while** — spoken at ~5 seconds if the answer hasn't started, ducked *over* the waiting sound rather than cutting it. Accepts a **list** of sounds: one plays at random, never the same twice in a row. Same server-side, live-apply semantics. Empty/`null` = never speak it. |
