...
| `audio_group` | unset | **Co-audible wake arbitration.** Give the same name (any string — `"loft"`, `"open-plan"`) to nodes close enough to hear the same "Hey Kenzy", and when several of them wake on one utterance, the server picks the node that heard you best (loudest wake phrase) and silently stands the others down — one chime, one answer, one pipeline, instead of every speaker answering at once. The decision happens inside the one-breath gate's held-chime window, so a losing node never chimes at all (with `wake_onset_ms: 0` it may chime before losing). **Works best with same-model speakerphones**: different models report the same voice at different levels (measured gaps of several dB), which can bias the pick toward the "hotter" microphone rather than the nearer one. Unset — the default — means this node never waits on, and can never lose to, another node. Live-applied. |
