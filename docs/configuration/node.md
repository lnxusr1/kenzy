...
| `tls_verify` | `false` | When connecting over `wss://` (a [TLS-enabled server](server.md#tls-configuration)): `false` = encrypted but the certificate is **not** verified — the right posture for a self-signed home-LAN cert, no CA install needed. `true` = require a certificate trusted by the system store. |
...
| `mic_volume` | *unset* | Capture (microphone) gain [0–100], written to the device's ALSA capture control. **Server-owned**, applies live and again at every audio init — so unlike hand-set `alsamixer` state it survives reboots and reinstalls. **Unset means the device's own gain is never touched**, and clearing the key stops managing it rather than reverting it. Linux-only (needs `amixer` from alsa-utils); when it can't apply — no ALSA card resolvable from the device name, no capture control, missing binary — the node page says exactly why. Devices with onboard AGC may absorb changes: verify against the calibration meter, which is also the tool for the two-rooms-hear-each-other tuning (lower the gain until far-room wake scores separate, then set `wakeword_threshold` in the gap). |
...
| `addons` | `{}` | Per-add-on settings for this node's installed add-ons, keyed by add-on id (e.g. `addons.ld2450.device`). Merged **per add-on** with `node_defaults`, edited from each add-on's own dashboard panel, applied live. Only meaningful when an add-on is installed — see [Add-ons](addons.md). |
...
| `sound_waiting` | `null` | WAV file played while waiting for the server response. Plays **once** — it does not repeat, so a short chime here stays a single chime rather than one every couple of seconds (the bundled `waiting.wav` is a ~26 s ambient bed that covers a long request on its own). While it is still playing the spoken processing cues below duck it underneath and it continues after them; once it has finished they are simply spoken. It stops the instant the reply begins. `null` (or an empty string) disables it — pure silence while waiting. Provide a filename or path to enable it. |
...
| `sound_thinking` | `"thinking.wav"` | Spoken status note ("Working on it.") when a reply is **taking a while** — spoken at ~5 seconds if the answer hasn't started, ducked *over* the waiting sound (rather than cutting it) whenever that is still playing. Accepts a **list** of sounds: one plays at random, never the same twice in a row. Same server-side, live-apply semantics. Empty/`null` = never speak it. |
| `sound_working` | `"working.wav"` | Second acknowledgement ("Still working on it.") for the long tail, ~8 seconds **after the first cue finishes** (a constant felt gap, however long the first clip is) — after it, Kenzy stays deliberately quiet until the answer (or the failure apology). Same pool/duck/live semantics as `sound_thinking`. Empty/`null` = skip it. |
...
| `wake_onset_ms` | `400` | **One-breath commands.** After the wake word fires, the ready chime is held this long. Keep talking — *"Hey Kenzy turn on the lights"*, no pause — and the chime is skipped entirely (it would land on top of your sentence); the command is captured from its first word and answered as usual, with the processing sound as the acknowledgement. Pause instead, and the chime plays and Kenzy listens exactly as before — just this many milliseconds later. `0` = the pre-gate behavior: chime the instant the wake word fires. Live-applied. |
...
| `audio_group` | unset | **Co-audible wake arbitration.** Give the same name (any string — `"loft"`, `"open-plan"`) to nodes close enough to hear the same "Hey Kenzy", and when several of them wake on one utterance, the server picks the node that heard you best (loudest wake phrase) and silently stands the others down — one chime, one answer, one pipeline, instead of every speaker answering at once. The decision happens inside the one-breath gate's held-chime window, so a losing node never chimes at all (with `wake_onset_ms: 0` it may chime before losing). **Same-model speakerphones across the group are strongly recommended** (not required): each model's automatic gain control judges loudness its own way — we've measured gaps of several dB between models hearing the same voice — and mixed hardware can bias the pick toward the "hotter" microphone rather than the nearer one. Identical devices judge alike; see [Choosing a Speakerphone](../choosing-a-speakerphone.md). **Every node in the group must run 5.1.1+** — an older node can't announce its wakes, so it keeps answering on its own regardless of the group (the server warns at each join when a grouped node is too old). **If your rooms chain** (a hallway that hears two rooms which can't hear each other), know that one group behaves as one unit — a wake anywhere in it ends the group's current conversation. Group the tightly-coupled cluster rather than the whole chain, unless one-conversation-at-a-time across those rooms is what you want. Unset — the default — means this node never waits on, and can never lose to, another node. Live-applied. |
