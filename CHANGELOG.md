...
- **Voice speaker enrollment.** Enroll a voice by speaking to a node — "Hey Kenzy, enroll me as Alice" — instead of only the `kenzy-enroll` CLI: Kenzy prompts for a few sentences, captures them through that room's mic, and registers them with the speaker service. **Off by default** (`allow_voice_enroll` in the speaker config — toggleable from the dashboard's Services → speaker, read live by the server); when enabled, anyone in earshot can enroll, so it's a deliberate opt-in (the docs warn this can bypass speaker-gated actions and recommend the CLI for people who can unlock things). `docs/speaker-enrollment.md` documents both paths.
