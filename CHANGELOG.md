...
- **Co-audible wake arbitration** (`audio_group`, opt-in per node). Two speakerphones close enough to hear the same "Hey Kenzy" both used to answer — in unison, like two butlers colliding in a doorway — and both ran a full pipeline (STT, speaker ID, the model, TTS) to produce one wanted answer and one duplicate. Now: give co-audible nodes the same `audio_group` name, and when several wake on one utterance the server picks the node that heard you best and silently stands the others down. One chime, one answer, one pipeline.
  ...
  Coverage completeness, each guard live-verified or pinned by test: a wake spoken *over* a node's own answer (the interrupt path) announces and arbitrates like any other; a losing node ignores its own wake-score tail for 0.8 s so the phrase it just lost can't re-open a session; and one utterance gets exactly one second of arbitration budget — after the 250 ms window closes, a node whose wake fires late on the same phrase is silently stood down instead of answering alone, with the lateness logged so slow hardware is visible rather than mysterious.

  The two timings that depend on hardware you own are settings, not constants: `arbitration.window_ms` (default 250 — widen it if the log shows dead-zone stragglers, meaning some device's wake fires later than the window) and `arbitration.deadzone_ms` (default 1000 — the per-utterance budget; shorten only for a much shorter custom wake phrase). Both editable from Settings, clamped to sane bounds, and the dead zone can never be shorter than the window it contains. The internal race-guard timings deliberately stay in code — they're correctness mechanics, not tuning.
...
> steps per node — a package upgrade deliberately won't add them for you.
> See [Upgrading → 5.0.4](https://docs.kenzy.ai/upgrading/#504-volume-buttons-on-existing-nodes).

### Added

- **The speakerphone's volume buttons work now.** A USB speakerphone's physical `+`/`−` buttons only ever worked on a desktop because a media-key daemon happened to be listening; on a headless node they pressed into the void. Opt in (`volume_buttons` on the node's config, off by default) and they move **Kenzy's canonical volume** — the same clamped, persisted value the dashboard, voice commands and MQTT see, changed through the same server-owned path, pushed back live. No ALSA-mixer side-channel, so there is never a second volume truth for surfaces to disagree over. Selection is deliberately narrow: `auto` accepts only an input endpoint that is a **USB sibling of the node's own audio device** advertising standard volume keys — a keyboard with media keys, or any other device in the room, can never qualify — and if more than one endpoint could, Kenzy picks nothing and names the candidates so you can set a stable device match (never an `/dev/input/eventN` path; those reshuffle across boots). On devices that report holds, a held button repeats at a rate-limited pace (a synthesized typematic — these HIDs generally don't autorepeat); devices that report a hold as an instant tap (the SP300U does) step once per press, honestly. Unplugging the speakerphone never disturbs listening or playback (the node just reports the endpoint absent and rescans); setup happens where you already pick the room's audio device — the guided audio wizard's device step offers the buttons whenever the device you chose has them, applied in the same step, with the three settings also in the node's config grid for later changes — and an **open config page follows along** — a volume changed by the buttons (or by voice, MQTT, or calibration) updates the page's slider live instead of leaving it stale, without disturbing anything you're mid-editing. Physical mute buttons are deliberately untouched — the tested devices mute their own microphone in hardware — and Kenzy's software mute is equally untouched in both directions. Linux-only, via the new `mediakeys` extra (`pip install "kenzy[node,mediakeys]"` — deliberately not part of the plain node install, so nodes that never opt in never grow its build-time dependency); without it, or anywhere else, the feature is a clean, labeled no-op. Both installers set it up **by default on nodes**, because the recommended room node is a USB speakerphone with echo cancellation and those carry volume keys: `install.sh` (node/all) and `kenzy-deploy` each install the extra with its build packages and add the node's user to the `input` group that `/dev/input` needs — reboot before enabling. `--no-media-keys`, or `media_keys: false` in `deploy.yaml`, opts out where a compiler or `/dev/input` access isn't wanted. Because evdev ships source-only on PyPI (no wheels, anywhere), it is deliberately still its own extra and installed in its own step, and a failed build is a **warning, not an abort** — you lose the buttons, never the install, and never an upgrade sweep on a host whose toolchain went missing. Upgrades keep it: a node's one-click upgrade carries the extra when evdev is already installed, and never adds it otherwise — so nodes that use the feature stay current and nodes that don't can't break on a source build.

## [5.0.3]

### Added

- **"Is anyone in the loft?" — and a "where is everyone" that never invents an empty house.** Room questions are answered from the occupancy model with its evidence and age spoken honestly: *"Looks like it — the presence sensor shows someone in the loft right now"*, *"Probably — there was activity about 4 minutes ago"*, *"There was some activity about 15 minutes ago, but I'm not sure anyone's still there."* When a recently heard voice identifies who, "who's in the loft" gets a name; otherwise the answer stays a strictly anonymous *someone* — motion is never a person (pets exist). And she never claims a room is **empty**: the tracker deliberately has no such state, so the strongest no is *"I haven't noticed anyone in the loft for a while — but I can't be sure it's empty."* The household summary gets the same honesty: a person whose tracker reads unknown or unreachable is said aloud — *"I'm not sure about Alex"* — never counted as away, never silently dropped, and *"nobody seems to be home"* can only be uttered when every single person actually reads away. Recently heard voices lead the answer (*"Alex was in the office just now"*), including people with no Home Assistant link at all. Rooms without a sensor say so plainly; a stale sensor picture admits it; occupancy off means room questions go to the model instead of pretending. All gated to recognized voices, all on the instant path for known rooms.

- **"Where is Alice?" gains a room and a freshness.** Until now the answer was the whole of what a Home Assistant person entity knows: "Alice is home." Kenzy has known more than that since 5.0.0 — the occupancy model remembers where each recognized voice was last heard — and this is the first feature to read it. Now: *"Alice is home — I last heard them in the office just now"* when she's been heard, *"Alice is home, but I haven't heard them recently"* when she hasn't, and a person whose record isn't linked to Home Assistant at all can still be answered from voice alone. The honesty rules are deliberate and load-bearing: the **age always rides along** (a room without a "when" is a lie by omission, and the memory of a voice fades from the answer entirely after a few minutes); **"away" ignores the voice memory** (Home Assistant's word is current, the voice is minutes old — the person just left, and repeating the echo would present stale evidence as fresh); and with occupancy disabled the answers stay exactly as they were — "I haven't heard them recently" is never said by a system that isn't listening. Still instant (the deterministic fast path composes the same sentence), still gated to recognized voices — where people are is household information.

- **"Where is Bobby?" now finds Bobbie.** Spoken names resolve forgivingly: the transcriber spells names its own way per speaker — a household's *Bobbie* arrives as "Bobby", *Vicki* as "Vikki", *Sarah* as "Sara" — and the old exact match killed the question outright ("I don't know anyone called Bobby") before anything was even looked up. Name matching now absorbs that drift (prefix-weighted, built for names), and the answer confirms the record's real spelling — ask about Bobby, hear "Bobbie is home." Two lines that will not be crossed: when two household names are genuinely close (*Jon* and *John*), Kenzy **asks which one you meant** instead of guessing — on voice she asks aloud and waits, in a typed channel the question is the reply; and a spoken name only ever picks the **subject** of a question — who is *speaking* is always decided by voice, so a mispronounced name can never reach into someone else's memory or secrets. For nicknames no spelling-match could bridge ("Bud" for Robert), person records gain an **"Also called"** field — editable on the person's People page, or as `aliases:` in `people.yaml`. Instant answers stay instant: the deterministic fast path uses the same resolution, and hands only the genuinely ambiguous cases to the conversational tier.
...
- **She knows which rooms have someone in them — room presence (the 5.0 headliner).** Kenzy now holds a live, per-room picture of where people are, built from your Home Assistant motion and presence sensors and from who she hears speaking in each room, and shows it on a new **Presence** tab: which rooms are occupied, what each belief actually rests on, and how fresh it is. Two kinds of evidence are tracked differently, because they mean different things — a motion sensor is a *pulse* (someone moved just now; confidence spikes and immediately starts decaying), while mmWave still-presence and a `person` entity's home/away are *levels* (they hold for as long as the sensor keeps asserting them, and go stale rather than confident when the connection to Home Assistant drops). Rooms read **unknown** until something says otherwise: "unknown" and "empty" are different claims, and only one of them is honest before any sensor has reported. **Nothing acts on any of it in 5.0** — no proactive speech, no change to how or where replies are delivered. It watches, you check it against the house, and the behaviors that depend on it arrive once the picture has earned it.

- **Home Assistant → Presence sensors — decide what counts as evidence.** Kenzy classifies your entities automatically from their device classes, which is right until it isn't: a hallway sensor the cat crosses nightly, or one aimed through a window at the street, will otherwise keep a room permanently "occupied". The new sub-tab shows what currently counts *and what could*, and you include or exclude per entity. It records only your **divergences** from the automatic classification, so improving Kenzy's defaults later still reaches every sensor you never touched. Kenzy's own `kenzy_*` entities are never evidence and never can be — she would otherwise believe every room was occupied the moment she spoke in it.

- **A room that goes missing stays on the dashboard instead of vanishing.** A node that lost the server used to disappear from the fleet entirely — from the list, the count, everything — which made a four-room house quietly becoming a three-room house look exactly like a house that only ever had three. It now stays listed as absent, with how long it has been gone, and becomes a fault on the Fleet page once it has been unreachable past `fleet.offline_alert_minutes` (default 5). Restarts and upgrades you asked for don't raise it, and nodes announce a deliberate shutdown themselves, so an ordinary `systemctl restart` — or a whole-fleet `kenzy-deploy` — stays quiet. With MQTT on, the same transition reaches Home Assistant, so you can have it tell your phone. A decommissioned node is dropped with **Forget** on its card.

### Fixed

- **A room that loses the server finds its way back.** A node that had been talking to the same server for days would still ask the network where it was on every single reconnect, over mDNS — and if that one multicast query went unanswered, or the underlying library hung, there was nothing else to try and no timeout to escape. One node sat orphaned for two days that way. It now remembers the address it last successfully joined and tries that first, with mDNS as the fallback rather than the only route; discovery is bounded, so a hung lookup can no longer park the reconnect loop; and the backoff only resets on an actual join, so a refused connection backs off properly instead of retrying every second until it trips the server's own rate limiter. **A node will also restart itself** if its reconnect loop stops running entirely, or after 30 minutes unable to register — the one thing in this release that acts without being asked. Tune or disable it under `watchdog` in `node_defaults` (`reexec_minutes: 0` = complain, never restart).

- **A node that can't hear you no longer chimes as though it can.** The activation chime played the instant the wake word fired, before anything checked whether there was a server to send the audio to — so an orphaned room answered every "Hey Kenzy" with a cheerful "I'm listening", then silence. From inside the room it looked like a working node, which is a large part of why that two-day outage went unnoticed. It now checks first, says so in the log, and stays quiet unless you point `sound_offline` at a distinct sound.

- **"bad/missing join token" now tells you which.** One warning covered three completely different faults — no proof at all, a proof signed with the wrong token, and a proof whose timestamp fell outside the freshness window — and they need opposite fixes. The last one is the common one in practice: a node's join is refused if its clock is more than two minutes off the server's, and a board without a battery-backed real-time clock boots with whatever its hardware clock drifted to. The server already knew the exact skew and was throwing it away; it now names the fault and the delta, and the node's own log carries the reason instead of discarding it. Node units are ordered after `time-sync.target`, and the installer offers to keep journals across reboots — because the logs that explain an outage should not die with the reboot you performed to fix it.
...
- **She starts talking before she's finished thinking — streaming replies (the 4.4 headliner).** With `streaming.enabled: true` in server.yaml (Settings-editable; off by default while it soaks), a non-fast reply no longer waits for the whole pipeline: the model's answer streams out sentence by sentence, each sentence is synthesized while the next is still being written, and the node starts playing the first one immediately — on a typical two-sentence answer, time-to-first-word after transcription dropped from ~11s to ~6s on the reference setup, with the tail of the reply synthesized entirely while the first sentence plays. Everything that made the buffered pipeline trustworthy carries over mechanically: a lockbox secret can never ride a streamed preview (secret exchanges deliver whole, after substitution — the value never transits a model or a delta), a provider that can't follow the streaming contract falls back to the classic path automatically, a mid-reply synthesis failure trails off honestly instead of erroring after speech began, the wake word still cancels everything instantly, and older nodes simply play the reply whole like before (the new protocol flag degrades gracefully). The reply contract's field order changed under the hood (spoken text last) so the header — tone, floor-holding — arrives before the text begins streaming.

- **"Working on it." … "Still working on it." — Kenzy acknowledges slow answers in layers, over a continuous waiting sound.** While a request is processed, the waiting sound now **loops** for the whole wait instead of running out mid-request, and spoken acknowledgements arrive on an escalating ladder: at ~5 seconds a calm "Working on it." (`sound_thinking`), then — if the answer *still* hasn't started ~8 seconds after that cue finishes — a reassuring "Still working on it." (the new `sound_working`), then deliberate silence until the answer (or the honest failure apology; there is no fake spoken timeout racing a still-running request). The second cue is timed from the end of the first, so the pause between them is a constant breath, never an accident of clip length. The cues no longer hard-cut the music: a new `cue` flag on the audio protocol has the node **duck the waiting sound underneath the spoken cue and carry on after it** — mixed off the real-time path, so the audio callback is untouched. Both cue keys accept a **list of sounds** (edited in the dashboard's per-node Sounds section): one plays at random, never the same twice in a row, so a busy kitchen doesn't hear the identical phrase forty times a day. Fast replies still hear none of this, everything stops the instant the reply starts, and cues only mark a *fresh* command: answering one of Kenzy's own follow-up questions stays cue-free (a "Working on it." mid-conversation breaks the rhythm), as do typed Assist requests and scheduled replays. A skill conducting a tight back-and-forth can waive the cues on its own questions (`ask(busy_cues=False)`) so a canned status phrase never barges its rhythm. Older nodes just play the cues the pre-4.4 way.

- **Knock-knock jokes, both directions.** Ask "tell me a knock-knock joke" and Kenzy performs the whole call-and-response out loud — "Knock knock!" / *Who's there?* / "Boo." / *Boo who?* / "Don't cry, it's only a joke!" — and she plays along when *you* start one ("Knock knock." → *Who's there?* → …). It's a small delight, but also the first skill built on the new `ask(busy_cues=False)`: a knock-knock exchange is pure conversational rhythm, so every turn waives the processing-cue ladder. Voice-only (a joke needs a room to hold the floor); on a typed channel she just writes one out.

- **"Regenerate spoken cues" — the pre-recorded phrases follow your voice.** The failure apology and the processing acknowledgements are pre-recorded WAVs — which meant a local-voice (Kokoro) install heard them in the *bundled* OpenAI voice, and any voice change left them stale. The phrases themselves are now config (`cues:` in server.yaml — edit them, add pool variants), and one click in Settings → Spoken cues re-records every phrase **through the currently configured TTS voice** into the sound library, points the fleet-wide defaults at the renders, and applies live — no restarts, no per-node work, and a failed render leaves the previous set untouched. Per-node `sound_*` overrides still win, and hand-recorded files still work everywhere they did.

### Fixed

- **A pre-release adversarial review of the streaming pipeline closed four gaps** (none shipped in a release; all reproduced by failing tests first, then fixed): a connection drop mid-streamed-reply left the node's player latched in streaming mode — the next reply played silently, and on half-duplex hardware the node would ignore wake words until restarted (the state machine now force-resets on teardown/reconnect, verified live by restarting the server mid-reply on the test rig); a second reply arriving within ~100ms of a streamed reply's end could be cut off and its session state stomped by the first reply's still-finishing drain task (new sessions now cancel the old drain first); a transport failure mid-stream played the error apology *on top of* an already-half-spoken reply, and a transient 5xx from the streaming endpoint failed the request outright instead of retrying buffered (policy now: speech already started ⇒ keep what was spoken and finish with whatever text arrived, never the cue; nothing arrived yet ⇒ silently retry on the classic path — but never a retry once events flowed, so a skill's side effects can't run twice); and a raw control character inside a local model's reply text (a literal newline — routine from prompt-tier models) poisoned conversation history with the raw JSON blob instead of the spoken text.
